...
public class LimitRaterInterceptor extends HandlerInterceptorAdapter {
    ...
    @Value("${smartcity.rateLimit.enable}")
    private boolean rateLimitEnable;

    @Value("${smartcity.rateLimit.limit}")
    private Integer limit;

    @Value("${smartcity.rateLimit.timeout}")
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response,
        ...
        if (os != null && StrUtil.isNotBlank(os.getBlacklist())) {
            ...
            for (String item : list) {
                if (item.equals(ip)) {
                    throw new SmartCityException("滚");
                }
            }
        }
        ...
        try {
            ...
            if (rateLimiter != null) {
                ...
                if (StrUtil.isBlank(token3)) {
                    throw new SmartCityException("当前访问人数太多啦，请稍后再试");
                }
            }
        } catch (Exception e) {
